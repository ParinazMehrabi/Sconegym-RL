...
class TorqueGaitGym(GaitGym):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        rew_keys.setdefault("upright_coeff", 0.0)
        ...
        n = len(self.model.actuators())
        self.prev_acts = np.zeros(n, dtype=np.float32)
        self.prev_excs = np.zeros(n, dtype=np.float32)
        self._current_action = np.zeros(n, dtype=np.float32)
    ...
    def step(self, action):
        ...
        self.time += self.step_size
        self.total_reward += reward
        self.prev_excs = self.prev_acts.copy()
        self.prev_acts = action.copy()
    ...
    def _setup_action_observation_spaces(self):
        ...
        self.action_space = gym.spaces.Box(
            low=-np.ones(shape=(num_act,), dtype=np.float32),
            high=np.ones(shape=(num_act,), dtype=np.float32),
            dtype=np.float32,
        )
 
        self.prev_acts = np.zeros(num_act, dtype=np.float32)
        self.prev_excs = np.zeros(num_act, dtype=np.float32)
        self._current_action = np.zeros(num_act, dtype=np.float32)
    ...
    def _get_obs(self):
        dof_values = np.asarray(
            self.model.dof_position_array(), dtype=np.float32
        ).copy()
 
        dof_vels = np.asarray(
            self.model.dof_velocity_array(), dtype=np.float32
        ).copy()
 
        # Remove absolute x position
        dof_values[1] = 0.0
 
        head_ori = np.asarray(
            self.head_body.orientation().array(), dtype=np.float32
        )
 
        head_ang_vel = np.asarray(
            self.head_body.ang_vel().array(), dtype=np.float32
        )
 
        feet_rel = np.asarray(
            self._get_feet_relative_position(), dtype=np.float32
        )
 
        acts = np.asarray(self.prev_acts, dtype=np.float32)
    ...
    def _update_rwd_dict(self):
        ...
        height = float(self.model.com_height().y)
        if vel <= self.target_vel:
        ...
        height_reward = float(np.clip(height - 0.05) / 0.4, 0.0, 1.0)
        smooth_penalty = float(np.mean(np.square(self._current_action - self.prev_action)))
        right_effort = float(np.abs(self._current_action[0] + np.abs(self.current_action[1])))
        left_effort = float(np.abs(self._current_action[2]) + np.abs(self._current_action[3]))
        asymmetry_penalty = abs(right_effort - left_effort)
        ...
        self.rwd_dict = {
            "gaussian_vel": self.vel_coeff * self._gaussian_plateau_vel(),
            "grf": self.grf_coeff * self._grf(),
            "smooth": self.smooth_coeff * self._action_smooth_cost(),
            "constr": self.joint_limit_coeff * self._joint_limit_torques(),
            "self_contact": self.self_contact_coeff * self._get_self_contact(),
            "upright": -self.upright_coeff * self._upright_cost(),
            "asymmetric": -getattr(self, "asymmetric_coeff",0.5) * asymmetry_penalty,
            "impact": -getattr(self, "impact_coeff", 0.02) * impact_jerk,
        }
        return self.rwd_dict
 
    def _action_smooth_cost(self):
        delta = self._current_action - self.prev_acts
        return float(np.mean(np.square(delta)))
 
    def _joint_limit_torques(self):
        ignore_joints = {"ankle_r", "ankle_l", "back"}
        joints = [j for j in self.model.joints() if j.name() not in ignore_joints]
        if not joints:
            return 0.0
        return float(
            np.mean([np.mean(np.abs(j.limit_torque().array())) for j in joints])
        )
 
    def _upright_cost(self):
        pelvis_tilt = self.model.dof_position_array()[0]
        return float(np.square(pelvis_tilt))
